...
function getNextOccurrences(recurringDays, startDate, daysAhead = 7) {
  ...
  occurrences.forEach((occ, index) => {
    console.log(
      `   ${index + 1}. ${occ.date.toDateString()} (${occ.dayName}) - ${occ.daysFromNow} days from now`
    );
  });
...
function testRollingWindowLogic() {
  ...
  console.log("==============================");
  const dailyResults = getNextOccurrences(
  ...
  const allDates = [...mwfResults, ...dailyResults, ...weekendResults, ...mondayResults].map(
    (result) => result.date.toDateString()
  );
  ...
  const outOfRangeDates = allResults.filter(
    (result) => result.date > maxDate || result.date < today
  );
