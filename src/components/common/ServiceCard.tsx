...
export default function ServiceCard({
  ...
  return (
    <div
      className={`relative bg-gradient-to-b ${getBgColor()} rounded-3xl overflow-hidden h-[400px] p-7 flex flex-col shadow-xl hover:shadow-2xl transition-all duration-300`}
    >
      {/* Card arc decoration */}
      ...
      <div className="bg-white rounded-2xl p-6 mb-auto w-4/5 shadow-md relative z-10">
        <div className="flex justify-between items-start">
          <div className="relative w-16 h-16">
            <Image src={imgUrl} alt={title} width={64} height={64} className="object-contain" />
          </div>
        </div>
          ...
          <ul className="mt-5 space-y-3">
            {bulletsItems.map((item, index) => (
              <li key={index} className="flex items-center">
                <span
                  className={`flex items-center justify-center w-5 h-5 rounded-full ${color === "blue" ? "bg-blue-100 text-blue-600" : "bg-pink-100 text-pink-600"} mr-3`}
                >
