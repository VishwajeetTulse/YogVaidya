...
export async function generateRecurringTimeSlots(
  ...
  try {
    ...
    console.log(`🔄 Generating recurring slots for ${generateForDays} days starting from ${generationStartDate.toDateString()}...`);
    console.log(`📅 Recurring days: ${recurringDays.join(', ')}`);
    console.log(`📅 Generation window: ${generationStartDate.toDateString()} to ${new Date(generationStartDate.getTime() + (generateForDays - 1) * 24 * 60 * 60 * 1000).toDateString()}`);
    ...
    if (slotsToCreate.length > 0) {
      ...
      const uniqueSlots = slotsToCreate.filter(slot => !existingTimes.has(slot.startTime.getTime()));
      ...
      if (uniqueSlots.length > 0) {
        ...
        console.log(`🎉 Generated ${uniqueSlots.length} recurring time slots for mentor ${mentorId}`);
    ...
    const actualSlotsCreated = slotsToCreate.length > 0 ? 
 ...
 */
export async function maintainRecurringSlots(): Promise<{ success: boolean; slotsGenerated: number; slotsDeleted: number; error?: string }> {
  try {
    ...
    for (const [templateKey, templateInfo] of templateMap) {
      ...
      const daysBeyondLatest = Math.ceil((latestSlot.getTime() - now.getTime()) / (24 * 60 * 60 * 1000));
      ...
      const slotsToGenerate = calculateMissingSlots(pattern.recurringDays, startGenerationDate, targetDaysAhead);
      
      if (slotsToGenerate > 0) {
        console.log(`   📅 Generating ${slotsToGenerate} additional slots starting from ${startGenerationDate.toDateString()}`);
    ...
    console.log(`✅ Daily maintenance completed. Generated ${totalGenerated} new slots, deleted ${slotsDeleted} old slots.`);
...
function calculateMissingSlots(recurringDays: string[], startDate: Date, daysAhead: number): number {
