...
function testFixedLogic() {
  ...
  console.log(`📊 Day of week: ${testSlotTime.toLocaleDateString('en-US', { weekday: 'long' }).toUpperCase()}`);
  ...
  const testScenarios = [
    ...
    { name: 'Daily Schedule', days: ['MONDAY', 'TUESDAY', 'WEDNESDAY', 'THURSDAY', 'FRIDAY', 'SATURDAY', 'SUNDAY'] },
  ...
  testScenarios.forEach(scenario => {
    ...
    slots.forEach((slot, index) => {
      console.log(`   ${index + 1}. ${slot.date.toDateString()} (${slot.dayName}) - ${slot.daysFromNow} days ahead`);
    });
  ...
  const mockExistingSlots = [
    { startTime: new Date(now.getTime() - 60 * 60 * 1000), endTime: new Date(now.getTime() - 30 * 60 * 1000) }, // 1 hour ago (should be deleted)
    { startTime: new Date(now.getTime() + 24 * 60 * 60 * 1000), endTime: new Date(now.getTime() + 25 * 60 * 60 * 1000) }, // Tomorrow (should be kept)
    { startTime: new Date(now.getTime() + 48 * 60 * 60 * 1000), endTime: new Date(now.getTime() + 49 * 60 * 60 * 1000) }, // Day after tomorrow (should be kept)
  ];
