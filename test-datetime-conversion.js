...
async function testDatetimeConversions() {
  ...
  try {
    ...
    if (scheduledSession) {
      const startResponse = await axios.post(`http://localhost:3000/api/sessions/${scheduledSession.id}/start`);
    ...
    if (ongoingSession) {
      const completeResponse = await axios.post(`http://localhost:3000/api/sessions/${ongoingSession.id}/complete`);
