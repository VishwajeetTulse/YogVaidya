/**
 * TypeScript Script to truncate (delete all data) from the Schedule collection
 * 
 * This script will:
...
async function truncateScheduleCollection(): Promise<void> {
  try {
    ...
    const sampleRecords: ScheduleSummary[] = await prisma.schedule.findMany({
      take: 3,
      select: {
        id: true,
        title: true,
...
// For interactive confirmation (optional enhancement)
async function promptForConfirmation(): Promise<boolean> {
  // This is a placeholder for interactive confirmation
  // In a real implementation, you might use readline-sync or similar
  console.log("💡 To add interactive confirmation, implement readline here");
